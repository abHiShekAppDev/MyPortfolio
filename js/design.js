function bplCode(){
    window.open("https://github.com/abHishekCoDeLab/BPL"); 
}

function helpMeCode(){
    window.open("https://github.com/abHishekCoDeLab/HelpMe");
}

function remindMeCode(){
     window.open("https://github.com/abHishekCoDeLab/RemindMe");
}

function getMyRepoCode(){
     window.open("https://github.com/abHishekCoDeLab/GetMyRepo");
}


function helpMeapk(){
     window.open("https://github.com/abHishekCoDeLab/HelpMe");
}

function remindMeapk(){
window.open("https://drive.google.com/file/d/1A95MpviEQdouTywPs7jOSjunRkOsknAu/view?usp=sharing");
}

function getMyRepoapk(){
   window.open("https://drive.google.com/file/d/1x6qnxjk9HYc-6mwjNRRIK68776p2Awbe/view?usp=sharing");
}

function bplApk(){ window.open("https://drive.google.com/file/d/1LYQu66D5K3XpOKAb2zwm1hq9zRWQuv5F/view");
}

function resume(){
    window.open("https://drive.google.com/file/d/1cLoXY5BbH789jhv06lhPr8cbxH3Q5SLn/view?usp=sharing"); 
}
